...
#pragma warning disable IDE0063 // Use simple 'using' statement: we want to control when the 'using' varibales go out of scope
            ...
            var separatedSyntaxListType = compilation.GetTypeByMetadataName("EV2.CodeAnalysis.Syntax.SeparatedSyntaxList`1");
            var syntaxNodeType = compilation.GetTypeByMetadataName("EV2.CodeAnalysis.Syntax.SyntaxNode");
            ...
            using (var indentedTextWriter = new IndentedTextWriter(stringWriter, indentString))
            {
                ...
                using (var nameSpaceCurly = new CurlyIndenter(indentedTextWriter, "namespace EV2.CodeAnalysis.Syntax"))
                {
